...
working directory with `/` replaced by `-`. Worktrees get their own sibling directories, so match
...
```bash
find <dirs> -name '*.jsonl' -mtime -$DAYS -exec grep -oh '<command-name>[^<]*</command-name>' {} + \
  | sort | uniq -c | sort -rn
```
...
Then look for what to **delete**: any command or skill in `.claude/` with zero invocations in the
window is a candidate. Deleting is a success condition, not a failure.
